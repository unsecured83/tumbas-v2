...
window.cariPesanan = async () => {
    const inputVal = document.getElementById('inputTrackOrderId').value.trim();
    if (!inputVal) {
        alert("Silakan masukkan Nomor Invoice atau No. WhatsApp!");
        return;
    }

    // Ubah tombol jadi loading
    const btnSearch = document.querySelector('#trackOrderPanel button[onclick="cariPesanan()"]');
    const originalText = btnSearch.innerHTML;
    btnSearch.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Mencari...';
    btnSearch.disabled = true;

    try {
        // Cek apakah input berupa angka (Nomor HP) atau teks (Invoice)
        // Regex ini mengecek jika input hanya berisi angka (dan opsional tanda + di depan)
        const isPhoneNumber = /^[0-9+]+$/.test(inputVal);
        let orderIdTarget = inputVal;

        if (isPhoneNumber) {
            // Jika itu nomor WA, cari di database transaksi (di dalam JSON detail_pembeli)
            const { data, error } = await supabaseClient
                .from('transaksi')
                .select('order_id')
                .eq('detail_pembeli->>phone', inputVal)
                .order('created_at', { ascending: false }) // Ambil pesanan yang paling baru
                .limit(1)
                .single();

            if (error || !data) {
                throw new Error("Pesanan dengan Nomor WhatsApp tersebut tidak ditemukan.");
            }
            // Jika ketemu, ganti target URL ke order_id asli dari database
            orderIdTarget = data.order_id;
        }

        // Langsung arahkan ke halaman status menggunakan order_id yang valid
        window.location.href = `status.html?id=${encodeURIComponent(orderIdTarget)}`;
        
    } catch (err) {
        alert(err.message || "Terjadi kesalahan saat mencari pesanan.");
    } finally {
